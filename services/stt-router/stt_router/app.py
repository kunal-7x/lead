        ...
        try:
            while True:
                ...
                if msg.get("bytes") is not None:
                    ...
                    if len(frame) == 0:
                        # Empty sentinel = end-of-utterance → transcribe
                        if not audio_buffer:
                            await websocket.send_text(json.dumps({
                        ...
                        # Batch fallback
                        assert _router is not None
                        result = await _router.transcribe(audio_bytes, lang, sid, tenant_id)
                        await websocket.send_text(json.dumps({
                            "type": "final",
                            "text": result.text,
