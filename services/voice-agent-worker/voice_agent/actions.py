...
from voice_agent.models import BrainOutput, SessionContext
...
async def handle_actions(
    ...
    base = {"session_id": ctx.session_id, "tenant_id": ctx.tenant_id,
            "lead_id": ctx.lead_id, "campaign_id": ctx.campaign_id}
    ...
    await publisher.publish(
        "lead.status.updated",
        json.dumps({**base, "status": brain.lead_status,
                    "score": brain.lead_score}).encode(),
    )
